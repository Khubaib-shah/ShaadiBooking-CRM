...
export default function DataTable<TData>({ columns, data, emptyMessage = 'No records found.' }: DataTableProps<TData>) {
  ...
  return (
    <div className="space-y-3">
      <div className="rounded-xl border border-[#e9ecef] bg-white p-3">
        <input
          value={globalFilter}
          onChange={(e) => setGlobalFilter(e.target.value)}
          placeholder="Search..."
          className="w-full rounded-lg border border-[#e9ecef] px-3 py-2 text-[13px] text-[#495057] focus:outline-none focus:ring-2 focus:ring-[#556ee6]/20"
        />
      </div>
